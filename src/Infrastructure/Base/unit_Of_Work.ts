...
export class Unit_Of_Work implements Iunit_Of_Work{
  ...
  constructor(@Inject('DATABASE_CONNECTION') private readonly async_Database_Connection: Connection) {
    ...
    this.current_Account_Repository = this.async_Database_Connection.getCustomRepository(Current_Account_Repository);
    this.saving_Account_Repository = this.async_Database_Connection.getCustomRepository(Saving_Account_Repository);
